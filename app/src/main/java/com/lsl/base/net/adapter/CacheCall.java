...
import com.lsl.base.net.callback.AbsCallbackWrapper;
        ...
        if (cacheMode == CacheMode.IF_NONE_CACHE_REQUEST) {
            ...
            if (cacheEntity != null && !cacheEntity.isExpire()) {
                ...
                if (data == null || headers == null) {
                    //由于没有序列化等原因,可能导致数据为空
                    sendFailResultCallback(true, rawCall, null, OkHttpExecption.INSTANCE("没有获取到缓存,或者缓存已经过期!"));
                } else {
        ...
        rawCall.enqueue(new okhttp3.Callback() {
            ...
            public void onResponse(okhttp3.Call call, okhttp3.Response response) throws IOException {
                ...
                if (responseCode == 304 && cacheMode == CacheMode.DEFAULT) {
                    if (cacheEntity == null) {
                        sendFailResultCallback(true, call, response, OkHttpExecption.INSTANCE("服务器响应码304，但是客户端没有缓存！"));
                    } else {
                        T data = cacheEntity.getData();
                        HttpHeaders headers = cacheEntity.getResponseHeaders();
                        if (data == null || headers == null) {
                            //由于没有序列化等原因,可能导致数据为空
                            sendFailResultCallback(true, call, response, OkHttpExecption.INSTANCE("没有获取到缓存,或者缓存已经过期!"));
                        } else {
                ...
                if (responseCode == 404 || responseCode >= 500) {
                    sendFailResultCallback(false, call, response, OkHttpExecption.INSTANCE("服务器数据异常!"));
                    return;
                }
